...
void timer_evt_set_frequency(struct timer_evt *tmevt, float hz);
...
void timer_evt_pause_counter(void *priv);
void timer_evt_resume_counter(void *priv);
...
static int adi_adxl36x_write(void *priv, unsigned len, uint8_t *data)
{
    ...
    // timer count is locked but events will keep ticking
    timer_evt_pause_counter(state); // pauses the counter timerfd and clears pending events
    ...
    if((state->power_ctl & REG_POWER_CTL_MEASURE_MASK) == 2){
        state->tme_init = 1;
        timer_evt_resume_counter(state);
    }else{
        state->tme_init = 0;
        timer_evt_pause_counter(state);
    }
    pthread_mutex_unlock(&state->lock);
    return i;
}
...
{
    adi_adxl36x_state_t *state = priv;
    float period = (1.0/state->odr);
    unsigned nsec = period * 1000000000;
...
void timer_evt_set_frequency(struct timer_evt *tmevt, float hz)
{
    ...
    epoll_ctl(state->epfd,  EPOLL_CTL_DEL, tmevt->tfd, tmevt->eevt); //remove timerfd events from epool
    ...
    timerfd_settime(tmevt->tfd, 0, &tmevt->itime, NULL);//set timer off and clears timerfd events
    epoll_ctl(state->epfd, EPOLL_CTL_ADD, tmevt->tfd, tmevt->eevt); //add the timer
    tmevt->itime.it_value.tv_nsec = nsec;
