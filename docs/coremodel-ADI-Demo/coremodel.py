...
class CoreModel(threading.Thread):
    ...
    def __init__(self, name, address, port, path):
        ...
        # coremodel_device_list_t *coremodel_list(void * cm);
        ...
        self.devlist = None # ctypes.POINTER(coremodel_device_list_t)
    ...
    def run(self):
        while not self.stop_event.is_set():
            if self.cycle_time == -1:
                self.libcm.coremodel_mainloop(self.cm, ctypes.c_longlong(300000))
            else:
                self.libcm.coremodel_mainloop(self.cm, ctypes.c_longlong(self.cycle_time))
